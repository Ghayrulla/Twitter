...
/*        self.delegate = self
        ...
        self.rowHeight = UITableViewAutomaticDimension //set thiis so autolayout will decide the height
        self.estimatedRowHeight = 200
*/
        TwitterClient.sharedInstance.getUser(screenName, success: { (user:User) -> () in
            self.user = user
            //set the user's attributes to the labels
            self.backgroundImage.setImageWithURL(user.profileHeaderUrl!)
            self.NameLabel.text = "@" + (user.screenName as! String)
            self.profileImage.setImageWithURL(user.profileUrl!)
            self.nameLabel.text = user.name as! String
            self.nTweets.text = String(user.tweetsCount!)
            self.nFollowing.text = String(user.followingCount!)
            self.nFollowers.text = String(user.followingCount!)
